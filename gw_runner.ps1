...
Write-Host ""
Write-Host ""
Write-Host "gw_runner.ps1 를 시작합니다" -ForegroundColor Magenta
			...
			switch ($userInput.ToLower()) {
				'y' {
					...
					catch
					{
						...
						Write-Host "gw_runner.ps1 실행을 종료합니다..." -ForegroundColor Red
						Write-Host " Made By hsyoon" -ForegroundColor Magenta
						exit 1
					}
				}
...
foreach ($module in $modules.Keys) {
	...
	if ($LASTEXITCODE -ne 0) {
		...
		Write-Host "[ERROR] gw_runner.ps1 실행을 종료합니다..." -ForegroundColor Red
		Write-Host " Made By hsyoon" -ForegroundColor Magenta
		exit $LASTEXITCODE
	} else {
